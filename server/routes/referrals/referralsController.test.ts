...
describe('POST /referrals/:id/completion-deadline', () => {
  ...
  describe('when the user inputs a valid date', () => {
    it('updates the referral on the backend and redirects to the referral form if the API call succeeds', async () => {
      ...
      expect(interventionsService.patchDraftReferral.mock.calls[0]).toEqual(['1', { completionDeadline: '2021-09-15' }])
    })
...
describe('GET /referrals/:id/complexity-level', () => {
  ...
  it('renders a form page', async () => {
    ...
    expect(interventionsService.getComplexityLevels.mock.calls[0]).toEqual(['b33c19d1-7414-4014-b543-e543e59c5b39'])
  })
  ...
  // TODO how do we (or indeed, do we) test what happens when the request has a complexity level - i.e. that the
  // correct radio is checked? Do we just do it as a presenter test?
})
