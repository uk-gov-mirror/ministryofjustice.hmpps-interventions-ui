...
export interface DeliusOffender {
  offenderId: string
  firstName: string
  surname: string
  dateOfBirth: string
...
export default class CommunityApiService {
  ...
  async getOffenderByCRN(crn: string): Promise<DeliusOffender> {
    const token = await this.hmppsAuthClient.getApiClientToken()
    logger.info(`getting details for offender with crn ${crn}`)
    return (await this.restClient(token).get({ path: `/secure/offenders/crn/${crn}` })) as DeliusOffender
  }
}
